...
### 8. App Composition — Adding Routes Directly (lines 210–227)
...
# Add Bearer auth middleware (F8)
app.add_middleware(BaseHTTPMiddleware, dispatch=_bearer_auth_middleware)
```

**Explain to students:**

- We add the extended card route **directly to `_a2a_app`** using
  `add_route()`, then expose `_a2a_app` as the final `app`.
- **Why not wrap in an outer Starlette with `Mount`?** This was the original
  approach, but it has a critical bug: **Starlette's `Mount()` does not
  propagate startup events to mounted sub-apps.** The `to_a2a()` function
  registers its A2A routes (JSON-RPC endpoint, agent card serving) during the
  Starlette `startup` lifecycle event. When wrapped in `Mount("/")`, that
  startup event never fires, and all A2A routes return 404.
- **The fix**: Add our custom route directly to the `_a2a_app` instance using
  `add_route()`. This avoids the `Mount` wrapper entirely, so `to_a2a()`'s
  startup handler runs correctly.
- **`app.add_middleware(...)`**: The middleware is added **after** construction.
  Starlette rebuilds its middleware stack lazily, so this works correctly.
  The middleware wraps **all** routes including the custom extended card route.
...
Bearer Auth Middleware (F8)
  ...
  └── else → verify JWT → 401 or pass through
        │
        ▼
  _a2a_app router (single Starlette app)
  │
  ├── /.well-known/agent.json → public agent card (registered by to_a2a at startup)
  ├── / (JSON-RPC) → message/send, message/stream, etc. (registered by to_a2a)
  └── /agents/authenticatedExtendedCard → _authenticated_extended_card()
```

**Teaching moment — the `Mount` startup bug**: This is a real pitfall when
composing Starlette apps. `to_a2a()` uses the `on_startup` event to register
routes dynamically. Starlette propagates startup events to its own app but
**not** to sub-apps mounted via `Mount()`. The fix is either: (a) add routes
to `_a2a_app` directly (what we do), or (b) use `app.router.lifespan_handler`
to manually propagate lifecycle events. Option (a) is simpler.

...
class TestGetWeather:
    ...
    @pytest.mark.asyncio
    async def test_mock_data_city_is_titlecased(self, monkeypatch):
    ...
    async def test_returns_error_on_api_failure(self, monkeypatch):
        ...
        with patch("httpx.AsyncClient", return_value=mock_client):
            result = await tools_module.get_weather("London")
...
class TestGetForecast:
    ...
    @pytest.mark.asyncio
    async def test_returns_mock_forecast_when_no_api_key(self, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_forecast_days_capped_at_5(self, monkeypatch):
        """Forecast should be capped at 5 days maximum."""
        monkeypatch.setenv("OPENWEATHERMAP_API_KEY", "")
        from weather_agent.tools import get_forecast
        result = await get_forecast("Paris", days=10)
